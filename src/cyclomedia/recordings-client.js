...
class RecordingsClient {
	...
	getRecordings(bounds, callback) {
		...
		// console.log('username:', this.username, 'password:', this.password, 'baseUrl:', this.baseUrl);
	    ...
	    success(data) {
				...
				// 	return;
				// }
				// console.log('recordingEls', recordingEls);

				let recordings = recordingEls.map(recordingEl => {
					...
					// const isAuthorized = recordingEl.getElementsByTagNameNS('*', 'isauthorized');//.firstChild.data;
					const isAuthorized = recordingEl.children[12].firstChild.data;
				...
				for (let recording of recordings) {
					if (recording.isAuthorized === "true") {
						recordingsEdit.push(recording);
					}
				}
				// console.log('recordings', recordings);
				console.log('recordingsEdit', recordingsEdit);

				callback(recordingsEdit);
			},
